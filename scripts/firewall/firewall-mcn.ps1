<#
.SYNOPSIS
    Script de configuration firewall Windows pour environnement MCN sécurisé
.DESCRIPTION
    Autorise UNIQUEMENT les ports RustDesk nécessaires (21115-21119)
.PARAMETER Action
    Action à effectuer: Enable, Disable, Status, Test
.EXAMPLE
    .\firewall-mcn.ps1 -Action Enable
    .\firewall-mcn.ps1 -Action Status
#>
...
function Enable-MCNFirewall {
    Write-Host "=== Configuration Firewall MCN ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    Get-NetFirewallRule -DisplayName "RustDesk MCN*" -ErrorAction SilentlyContinue | Remove-NetFirewallRule
    ...
    # hbbs - NAT type test
    New-NetFirewallRule -DisplayName "RustDesk MCN - hbbs NAT Test (21115)" `
        -Direction Inbound `
        -Protocol TCP `
        -LocalPort 21115 `
        -Action Allow `
    ...
    New-NetFirewallRule -DisplayName "RustDesk MCN - hbbs Registration TCP (21116)" `
        -Direction Inbound `
        -Protocol TCP `
        -LocalPort 21116 `
        -Action Allow `
    ...
    New-NetFirewallRule -DisplayName "RustDesk MCN - hbbs Registration UDP (21116)" `
        -Direction Inbound `
        -Protocol UDP `
        -LocalPort 21116 `
        -Action Allow `
    ...
    # hbbr - Relay
    New-NetFirewallRule -DisplayName "RustDesk MCN - hbbr Relay (21117)" `
        -Direction Inbound `
        -Protocol TCP `
        -LocalPort 21117 `
        -Action Allow `
    ...
    # hbbs - WebSocket
    New-NetFirewallRule -DisplayName "RustDesk MCN - hbbs WebSocket (21118)" `
        -Direction Inbound `
        -Protocol TCP `
        -LocalPort 21118 `
        -Action Allow `
    ...
    New-NetFirewallRule -DisplayName "RustDesk MCN - hbbr WebSocket (21119)" `
        -Direction Inbound `
        -Protocol TCP `
        -LocalPort 21119 `
        -Action Allow `
    ...
    # HTTPS API (optionnel)
    New-NetFirewallRule -DisplayName "RustDesk MCN - API HTTPS (443)" `
        -Direction Inbound `
        -Protocol TCP `
        -LocalPort 443 `
        -Action Allow `
    ...
    Get-NetFirewallRule -DisplayName "RustDesk MCN*" | Enable-NetFirewallRule
    
    Write-Host ""
    Write-Host "✅ Firewall MCN activé avec succès" -ForegroundColor Green
    Write-Host ""
...
function Disable-MCNFirewall {
    Write-Host "=== Désactivation Firewall MCN ===" -ForegroundColor Yellow
    Write-Host ""
    ...
    # Supprimer toutes les règles RustDesk MCN
    Write-Host "Suppression des règles RustDesk MCN..." -ForegroundColor Yellow
    Get-NetFirewallRule -DisplayName "RustDesk MCN*" -ErrorAction SilentlyContinue | Remove-NetFirewallRule
    
    Write-Host ""
    Write-Host "✅ Firewall MCN désactivé" -ForegroundColor Green
    Write-Host "⚠️  ATTENTION: Les ports RustDesk ne sont plus protégés!" -ForegroundColor Red
}

function Show-MCNFirewallStatus {
    Write-Host "=== Statut Firewall MCN ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "Règles RustDesk MCN:" -ForegroundColor Yellow
    $rules = Get-NetFirewallRule -DisplayName "RustDesk MCN*" -ErrorAction SilentlyContinue
    ...
    if ($rules) {
        ...
        Write-Host "Aucune règle RustDesk MCN trouvée" -ForegroundColor Red
    }
...
function Test-MCNFirewall {
    Write-Host "=== Test Firewall MCN ===" -ForegroundColor Cyan
    Write-Host ""
    ...
    foreach ($port in $ports) {
        $rule = Get-NetFirewallRule -DisplayName "RustDesk MCN*" | 
